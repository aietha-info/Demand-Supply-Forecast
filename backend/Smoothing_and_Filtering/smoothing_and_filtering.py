...
def main(data_obj):
    ...
    with st.expander(label="How to use", expanded=True):
        st.info("""
                ...
                """)
        st.warning(
            "**No matter if you did any changes or not -- do not forget to press 'Finalize' button after you are done with 'Smoothing and filtering'!**")
    ...
    if dp_method == 'Remove outliers':
        ...
        if rmo_radio == 'Std':
            ...
            with st.container():
                ...
                with cc3:
                    scatter_column2 = st.selectbox('Select 2nd column for the scatter plot',
                                                   [s for s in columns_list if s != selected_column])
                with cc4:
                    st.write(" ")
                    st.write(" ")
            ...
            with st.container():
                ...
                try:
                    with cc1:
                        ...
                        col_group = st.multiselect('Group by', current_df.columns,
                                                   default=[current_df.columns[0], current_df.columns[1]])
