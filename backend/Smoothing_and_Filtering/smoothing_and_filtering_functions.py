...
class Remove_Outliers():
    ...
    def removeOutlier(df, columnName, n):
        ...
        toVal = mean + n * std
        filtered = df[
            (df[columnName] >= fromVal) & (df[columnName] <= toVal)]  # apply the filtering formula to the column
        return filtered
    ...
    def removeOutlier_q(df, columnName, n1, n2):
        ...
        :return pandas.core.frame.DataFrame: filtered dataframe
        """
        lower_quantile, upper_quantile = df[columnName].quantile(
            [n1, n2])  # quantiles are generally expressed as a fraction (from 0 to 1)
